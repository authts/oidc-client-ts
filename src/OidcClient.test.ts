...
describe("OidcClient", () => {
    ...
    describe("createSigninRequest", () => {
        ...
        it("should include a dpop key thumbprint in params if dpop is enabled and bind_authorization_code is true", async () => {
            ...
            // assert
            const url = request.url;
            expect(url).toContain("dpop_jkt");
        });

        it("should not include a dpop key thumbprint in params if dpop is enabled and bind_authorization_code is false", async () => {
            // arrange
            jest.spyOn(subject.metadataService, "getAuthorizationEndpoint").mockImplementation(() => Promise.resolve("http://sts/authorize"));
            subject.settings.dpopSettings.enabled = true;
            subject.settings.dpopSettings.bind_authorization_code = false;
            ...
            // assert
            const url = request.url;
            expect(url).not.toContain("dpop_jkt");
        });

        it("should not include a dpop key thumbprint in params if dpop is disabled", async () => {
            // arrange
            jest.spyOn(subject.metadataService, "getAuthorizationEndpoint").mockImplementation(() => Promise.resolve("http://sts/authorize"));
            subject.settings.dpopSettings.enabled = false;
