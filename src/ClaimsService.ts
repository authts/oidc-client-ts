...
export class ClaimsService {
    ...
    private legacyMergeClaims(claims1: UserProfile, claims2: JwtClaims): UserProfile {
    ...
    public mergeClaims(claims1: JwtClaims | UserProfile, claims2: JwtClaims): JwtClaims | UserProfile {
        if (this._settings.mergeClaimsLegacyBehavior) {
        ...
        for (const [claim, value] of Object.entries(claims2)) {
            if (result[claim] !== value) {
                if (Array.isArray(value)) {
                    result[claim] = value;
                } else if (typeof result[claim] === "object" && typeof value === "object") {
